...
class ALGORITHM_API MLimagePublic
    ...
    public:
        ...
        bool GetAllMinLoc(cv::Mat image, cv::Mat templ, double sim, cv::Scalar mask, std::vector<cv::Point> *all_min_loc,
                          int num = 1);
        void readROItxtFile(std::string filepath, std::vector<cv::Rect>& roiArea);
        void readPointstxtFile(std::string filepath, std::vector<cv::Point2f>& points);
        void writePointsFile(std::string fileName, std::vector<cv::Point2f>points);
        void writeROIFile(std::string fileName, std::vector<cv::Rect>roiRect);
        cv::Mat readCSVToMat(const std::string &filename);
        void writeMatTOCSV(std::string filename, cv::Mat m);
        std::vector<int> findPeaks(cv::Mat data, double minHeight, int minWidth, double minProminence);
        ...
        std::vector<cv::Point> drawline(double k, double b);
        std::vector<cv::Point> drawline(std::vector<double> line);
        double median(std::vector<double>valVec);
        double median(double a1, double a2, double a3);
        std::vector<double> LineInf(cv::Point2f p1, cv::Point2f p2);
        ...
        bool sort_user(const std::vector<double> x, std::vector<double>& result, std::vector<int>& index);
        ...
        std::vector<double> line_fit(cv::Mat abs_grad, int flag);
        std::vector<double> line_fit_orthogonal(cv::Mat abs_grad);
        ...
        std::vector<double>unique(std::vector<double>xVec,int thresh);
        int findIndex(std::vector<cv::Point2f>pts, cv::Point2f pt0,double thresh);
        int findNeighborNum(std::vector<double>xVec, double x0, double thresh = 25);
        int findNeighborNum(std::vector<double> xVec, double x0, std::vector<int>&inVec, double thresh = 25);

        std::vector<cv::Point2f> pointsClusters(std::vector<cv::Point2f> pts, double thresh = 25);
        std::vector<double>dataClusters(std::vector<double>data,std::vector<int>&vecList,double thresh=25);
        std::vector<double> dataIndexMap(std::vector<double> data, std::vector<cv::Point> P, cv::Mat &indexMap, double thresh = 50);
        cv::Mat generatePointsIndexMap1(std::vector<cv::Point2f>pts,bool extendFlag=false,double thresh=50);
       
        cv::Mat generatePointsIndexMap(std::vector<cv::Point2f> pts, bool extendFlag = false, double thresh = 50);

        std::vector<cv::Point> convecHull(std::vector<cv::Point> &points); 
        double getDistancePL(cv::Point a, cv::Point b, cv::Point c);
        std::vector<int> sortIndex(std::vector<double> xVec);
        cv::Point2f getPointsCenter(std::vector<cv::Point2f>pts);
        int findNeartVal(std::vector<double> xVec, double x0);
        ...
        std::vector<cv::Point2f> rotatePoints(std::vector<cv::Point2f> pts, cv::Point2f center, double angle);
