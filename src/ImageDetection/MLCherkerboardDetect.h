#pragma once
#ifndef MLALGORITHM_MLCHESSBOARDDETECT_H_
#define MLALGORITHM_MLCHESSBOARDDETECT_H_
#ifdef ALGORITHM_EXPORTS
#define ALGORITHM_API __declspec(dllexport)
#else
#define ALGORITHM_API __declspec(dllimport)
#endif
#include <opencv2/core/base.hpp>
#include <opencv2/core/mat.hpp>
#include <vector>
#include "ml_image_public.h"
namespace MLImageDetection
{
	struct  CheckerboardRe
	{
        std::vector<cv::Point2f>pts;
        cv::Mat indexMap;
        cv::Mat xLocMat;
        cv::Mat yLocMat;
        std::vector<cv::Rect>rectVec;
        cv::Point2f center;
        cv::Size boardSize;
        cv::Mat img_draw;
        bool flag = true;
       std::string errMsg = "";
	};
    class ALGORITHM_API MLCherkerboardDetect :MLimagePublic
	{
	public:
		MLCherkerboardDetect();
		~MLCherkerboardDetect();
	public:
		void SetChessboardSize(cv::Size size);
		void SetCRAreaPercent(double percent);
        void SetChecssboardPointsClusters( double thresh);
        void SetChessboardxyClassification(double thresh);
        void SetChessboardUpdateFlag(bool flag);
        void SetRotationAngle(double angle);

        CheckerboardRe detectChessboardCorner(cv::Mat img,double craRatio,   int binNum = 1);
        CheckerboardRe detectChessboardCorner1(cv::Mat img, double crRatio,   int binNum = 1);
        CheckerboardRe detectChessboardHoughLine(cv::Mat img, double crRatio, int binNum = 1);
        CheckerboardRe detectChessboardTemplate(cv::Mat img, double crRatio, int binNum = 1);
        CheckerboardRe detectChessboardContour(cv::Mat img, double crRatio, int binNum = 1);
        CheckerboardRe detectChessboardTemplate1(cv::Mat img, double crRatio, int binNum = 1);
        CheckerboardRe detectChessboardHist(cv::Mat img, double crRatio);


		std::vector<cv::Point2f> matchTemplateMaxLocs(cv::Mat img, cv::Mat templ, double score = 0.7, int bin = 1);
		std::vector<cv::Rect> getCRROI(std::vector<cv::Point2f>corners, cv::Mat indexMap, std::vector<cv::Point2f>& centerVec);
    private:
        bool getNewROIRect(std::vector<cv::Rect> roiVecP, std::vector<cv::Rect> roiVecN, std::vector<cv::Rect>& NewroiVecP, std::vector<cv::Rect>& NewroiVecN);
        bool ReoderROIVec(std::vector<cv::Rect> roiVecP, std::vector<cv::Rect> roiVecN, std::vector<cv::Rect>& NewroiVecP,
            std::vector<cv::Rect>& NewroiVecN);
        int findNegtiveNum(std::vector<int> list);
        int findMapNegNum(cv::Mat list);
        bool searchColIndex(cv::Mat indexMap, int& start, int& end);
        bool searchRowIndex(cv::Mat indexMap, int& start, int& end);
        cv::Mat updateIndexMap(cv::Mat indexMap, std::vector<cv::Point2f> cornersPoint,
            std::vector<cv::Point2f>& cornersPointNew);
    public:
        cv::Mat updateIndexMapKL(cv::Mat indexMap, std::vector<cv::Point2f> cornersPoint,
            std::vector<cv::Point2f>& cornersPointNew);
        cv::Mat updateIndexMapKLRectangle(cv::Mat indexMap, std::vector<cv::Point2f> cornersPoint, std::vector<cv::Point2f>& cornersPointNew);

        cv::RotatedRect getCherkerBorder(cv::Mat img, cv::Rect& rect);

    private:
        bool getXYcoordinate(cv::Mat indexMap, std::vector<cv::Point2f> cornersPoint, cv::Mat& x, cv::Mat& y);
        bool getCenter(cv::Mat ip, cv::Mat& p, cv::Mat& px1, cv::Mat& px2, cv::Mat& px3, cv::Mat& px4);
        cv::Mat getArea(cv::Mat x1, cv::Mat x2, cv::Mat x3, cv::Mat x4, cv::Mat y1, cv::Mat y2, cv::Mat y3, cv::Mat y4);
        int findNearstList(std::vector<cv::Rect> roiVecN, cv::Rect temp);
        std::vector<double> CalculateROIValue(cv::Mat img, std::vector<cv::Rect> roiVec);
        std::vector<double> CalculateCR(std::vector<double> posValue, std::vector<double> negValue);
        void updateImgdraw(cv::Mat &imgdraw, std::vector<cv::Point2f>pts);
        cv::Point2f getPtsCenter(std::vector<cv::Point2f>pts);

    private:
        cv::Rect m_rect = cv::Rect(0, 0, -1, -1);
        double m_CRAreaPercent = 0.25;
        int m_chessAlgortithm = 1;
        int m_pointsClusters = 50;
        int m_xyClassification = 150;
        int m_singlecheckerWidth = 100;
        bool m_update = true;
        cv::Size m_boardsize = cv::Size(5, 5);
        int m_binNum = 1;
        double m_rotationAngle = 0;
	};
}
#endif