	...
	int binNum)
{
	...
	if (img.data != NULL)
	{
		...
		//string filepath1 = "E:/project/IQMetric/x64/Debug/config/templateImg/chessboardtemplate1.tif";
		string filepath1 = "./config/templateImg/chessboardtemplate1.tif";
		cv::Mat templ = cv::imread(filepath1, 0);
		if(templ.empty())
			templ = cv::imread("../config/templateImg/chessboardtemplate1.tif", 0);
		if (templ.data == NULL)
		{
		...
		vector<cv::Point2f> pts1 = matchTemplateMaxLocs(img, templ, 0.5, binNum);
