...
vector<cv::Point2f> MLImageDetection::MLimagePublic::pointsClusters(vector<cv::Point2f> pts, double thresh)
{
	vector<double> dis1;
	double k[9000] = { 0 };
	cv::Mat kMat = cv::Mat::ones(cv::Size(1, 9000), CV_16UC1);
	kMat = -kMat;
	//   cv::Mat dis = cv::Mat::zeros(cv::Size(100, 100), CV_32FC1);
	cv::Mat index = cv::Mat::ones(cv::Size(9000, 9000), CV_16UC1);
	cv::Mat xValue = cv::Mat::ones(cv::Size(9000, 9000), CV_32FC1);
	cv::Mat yValue = cv::Mat::ones(cv::Size(9000, 9000), CV_32FC1);
	index = -index;
	xValue = -xValue;
	yValue = -yValue;
	if (pts.size() > 0 & pts.size() < 9000)
	{
