 ...
 namespace MLImageDetection
{
	...
	struct PointSet
	{
		...
		int angle1;
		std::vector<cv::Point2f>ptsOther;
		int angle2;
		bool flag = false;
	};
	struct Peak {
	...
	public:
		...
		std::vector<cv::Point> get_roi_center();
		std::vector<cv::Point> get_newroi_center();
		int get_line_width();
		cv::Mat getImgDraw();
		cv::Point2f getRotationAngle();
		std::vector<double> GetVerticalLine();
		std::vector<double> GetHorizontalLine();
		...
		int calLine(std::vector<cv::Vec4i>& lines, std::vector<float>& ks, float& a, float& b, float& c);


	private:
		int m_binNum = 1;
		std::vector<float> m_angle1, m_angle2;
		double m_thetaH = 0;
		double m_thetaV = 0;
		...
		std::vector<cv::Point> new_roi_center;
		std::vector<cv::Point> find_roi_center(cv::Point2f center, float a1, float b1, float c1, float a2, float b2, float c2);
		void sepLines(std::vector<cv::Vec4i>& lines, std::vector<cv::Vec4i>& line1, std::vector<float>& ks1, std::vector<cv::Vec4i>& line2,
			std::vector<float>& ks2);
		cv::Vec4f verticalLineFit(cv::Mat img, bool fitflag);
		PointSet verticalScan(cv::Mat img);
		PointSet horizontalScan(cv::Mat img);
		float findGaussianFitMaxValue(std::vector<cv::Point2f>pts);
		...
		std::vector<double>linespace(double start, double end, double step);
		std::vector<cv::Point2f> pointFinterByAngle(std::vector<cv::Point2f>pts, double thresh = 5);
		std::vector<cv::Point2f> pointClusterByAngle(std::vector<cv::Point2f>pts, double thresh = 5);
		PointSet pointFilterAngle(std::vector<cv::Point2f>pts, double thresh = 5);
		std::vector<int> findPeaks(cv::Mat data, std::vector<Peak>& peaks, double minHeight, int minWidth, double minProminence);
		std::vector<int>findPeaksByContour(cv::Mat rowMat);
		std::vector<int>findPeakByMaxLoc(cv::Mat rowMat);
		std::vector<int>findPeaks1(cv::Mat rowMat, double minpeakdistance, double minpeakheight);
